...
class TaskSet {
  ...
  float MarkPerformedTask(CPUState &state, uint32_t output, size_t task_id,
    ...
    if (config.SGPOrganismType() == NUTRIENT) {
      if (config.NUTRIENT_TYPE() == MUTUALIST){
        ...
        // if sym has host 
        else if(state.organism->GetHost() != nullptr){
          // sanity check if symbiont 
          emp_assert(!state.organism->IsHost());
          // tasks don't match
          emp::Ptr<SGPHost> host_ptr = static_cast<SGPHost*>(state.organism->GetHost().Raw());
          const bool host_performed_task = host_ptr->GetCPU().state.tasks_performed->Get(task_id);
          // if task match
          if (host_performed_task){
            const double points_to_donate = score * config.NUTRIENT_SYM_MATCHTASK_STOLEN();
            const double points_to_keep = score - points_to_donate;
            emp_assert(config.NUTRIENT_SYM_MATCHTASK_STOLEN() <= 1.0);
            emp_assert(config.NUTRIENT_SYM_MATCHTASK_STOLEN() >= 0.0);
            emp_assert(points_to_keep >= 0);
            // host_ptr->AddPoints(points_to_donate);
            // state.organism->AddPoints(points_to_keep);
            score = points_to_keep;

            // if tasks don't
          } else {
            // donate resources to host
            // const double 
        ...
        if (state.organism->IsHost()){
          ...
          // symbiont 
          if (host_performed_task){
              const double points_stolen = score * config.NUTRIENT_SYM_MATCHTASK_STOLEN();
              const double points_to_donate = points_stolen - score;
              emp_assert(config.NUTRIENT_SYM_MATCHTASK_STOLEN() <= 1.0);
              emp_assert(config.NUTRIENT_SYM_MATCHTASK_STOLEN() >= 0.0);
              emp_assert(points_to_donate >= 0);
              // host_ptr->AddPoints(points_to_donate);
              // state.organism->AddPoints(points_to_keep);
              score = points_to_donate;
