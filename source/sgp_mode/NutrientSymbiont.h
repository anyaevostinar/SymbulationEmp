// #ifndef NUTRIENTSYMBIONT_H
// #define NUTRIENTSYMBIONT_H

// #include "NutrientSymbiont.h"
...
class NutrientSymbiont : public Symbiont {
public:
  using this_t = NutrientSymbiont<HW_SPEC_T>;
...
public:
  /**
   * Constructs a new NutrientSymbiont as an ancestor organism, with either a random
  ...
  /**
   * Constructs an NutrientSymbiont with a copy of the provided genome.
   */
  NutrientSymbiont(
    emp::Ptr<emp::Random> _random,
    emp::Ptr<world_t> _world,
  ...
  NutrientSymbiont(const NutrientSymbiont& symbiont) :
    Symbiont(symbiont),
  ...
  bool operator<(const Organism& other) const {
    if (const NutrientSymbiont* sgp = dynamic_cast<const NutrientSymbiont*>(&other)) {
  ...
  bool operator<(const NutrientSymbiont& other) const {
  ...
  void SetHost(emp::Ptr<Organism> host) {
    emp_assert(host.DynamicCast<host_t>(), "NutrientSymbiont must have an SGPHost host");
   ...
   *
   * Purpose: To produce a new NutrientSymbiont
   */
  emp::Ptr<Organism> Reproduce() {
    // emp::Ptr<NutrientSymbiont> sym_baby = Symbiont::Reproduce().DynamicCast<NutrientSymbiont>();
    // NOTE - should be able to static cast here
    emp::Ptr<NutrientSymbiont> sym_baby = static_cast<NutrientSymbiont*>(Symbiont::Reproduce().Raw());
...
// NutrientSymbiont& AsNutrientSymbiont(emp::Ptr<Organism> org_ptr) {
//   return *(static_cast<NutrientSymbiont*>(org_ptr.Raw()));
// }

// const NutrientSymbiont& AsNutrientSymbiont(emp::Ptr<Organism> org_ptr) const {
//   return *(static_cast<NutrientSymbiont*>(org_ptr.Raw()));
// }
