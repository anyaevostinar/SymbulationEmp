...
void worldSetup(emp::Ptr<SGPWorld> world, emp::Ptr<SymConfigBase> my_config) {
  // params
  ...
  if (my_config->HOST_INT() == -2 && !my_config->COMPETITION_MODE())
    random_phen_host = true;
  if (my_config->SYM_INT() == -2)
    random_phen_sym = true;

  if (my_config->GRID() == 0) {
    world->SetPopStruct_Mixed(false);
  } else {
    world->SetPopStruct_Grid(my_config->GRID_X(), my_config->GRID_Y(), false);
  }
  ...
  for (size_t i = 0; i < POP_SIZE; i++) {
    ...
    // Currently hacked because there isn't an AddOrg function, but there
    // probably should be
    if (my_config->GRID()) {
  ...
  for (int j = 0; j < total_syms; j++) {
    ...
    if (random_phen_sym) {
      ...
      sym_int = my_config->SYM_INT();
    }

    emp::Ptr<SGPSymbiont> new_sym =
        emp::NewPtr<SGPSymbiont>(&random, world, my_config, sym_int, 0);
