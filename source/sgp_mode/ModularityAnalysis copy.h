...
#include <math.h>
  ...
  */
  ...
  //get ReducedProgramRepresentations
  //get ReducedPrograms
  //GetNecessaryInstructions

    sgpl::Program<spec> GetNecessaryInstructions(SGPHost* test_host,size_t test_task_id, TaskSet task_passer){
    ...
    for (size_t j=0;j<all_tasks.NumTasks();++j){
            ...
            if(all_tasks.CanPerformTask(condition, j)){
...
emp::vector<sgpl::Program<spec>> GetReducedPrograms(SGPHost* host){
    ...
    host->GetCPU().RunCPUStep(condition.location, 100);
