...
public:
  ...
  emp::DataMonitor<int>& GetHostCountDataNode() {
    if(!data_node_hostcount) {
      ...
      OnUpdate([this](size_t){ // this lambda function is called each time Update() in the World.h definition is called
        data_node_hostcount -> Reset(); 
        for (size_t i = 0; i< pop.size(); i++)
          if(IsOccupied(i))
            data_node_hostcount->AddDatum(1); // since we are getting count of host. Each loop we just add one.
      });
    }
