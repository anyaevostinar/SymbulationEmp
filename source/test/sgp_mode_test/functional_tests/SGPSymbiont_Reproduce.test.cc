...
//Porting in progress but "test" currently seg faulting

// TEST_CASE("SGPSymbiont Reproduce", "[sgp][sgp-functional][test]") {
...
//       WHEN("Parental task tracking is set to parent-all") {
...
//           REQUIRE(sym_parent->GetHardware().GetCPUState().GetTasksPerformed().Get(0));
//           REQUIRE(sym_parent->GetHardware().GetCPUState().GetTasksPerformed().CountOnes() == 1);

//           REQUIRE(sym_baby->GetHardware().GetCPUState().GetParentTasksPerformed().Get(0));
...
//                 world.Update();
//               }
//               THEN("Symbiont does not vertically transmits"){
//                 REQUIRE(world.GetVerticalTransmissionAttemptCount().GetTotal() == 1);
...
//             host->GetCPU().state.parent_or_current_tasks_performed->Set(0);
