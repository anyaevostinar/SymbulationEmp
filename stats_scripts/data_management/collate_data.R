...
frame <- frame[rep(1, nrow(host_data)),]
##Loop through all recorded times (This loop is placed very poorly for maximizing efficiency)
#for(i in 1:nrow(host_data)) {
frame$UPS <- as.numeric(host_data$update)#Overwrite previous UPS value
frame$host_count <- as.numeric(host_data$count)
frame$sym_count <- as.numeric(sym_data$count)

#Compute desired values
frame$survival <- frame$host_count / frame$POP
frame$moi <- frame$sym_count/frame$host_count
#	if(frame[i,]$host_count[1] == 0) {
#		frame[i,]$moi <- c(0)
#	} else {
#		frame[i,]$moi <- frame[i,]$sym_count / frame[i,]$host_count
#	}
