...
#Read in the data
initial_data <- read.table("12-12-22-LimitedRes/munged_basic.dat", h=T)
initial_data$task <- factor(initial_data$task, levels=c("NOT", "NAND", "AND", "ORN", "OR", "ANDN", "NOR", "XOR", "EQU"))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("Long_MOI0.0", "Absent", x)}))
initial_data[2] <- data.frame(lapply(initial_data[2], function(x) {gsub("Long_MOI1.0", "Present", x)}))
#final_update <- subset(initial_data, update == "40000")
final_update <- subset(initial_data, update == "79900")
...
#regular box plot
ggplot(data=host_data, aes(x=treatment, y=count, color=treatment)) + geom_boxplot(alpha=0.5, outlier.size=1) + ylab("Boolean Logic Operation Count Final Update") + xlab("Parasites Present or Absent") + theme(panel.background = element_rect(fill='white', colour='black')) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + guides(fill=FALSE) + scale_color_manual(name="Parasites", values=viridis(3)) + facet_wrap(~task, scales = "free") + scale_x_discrete(guide=guide_axis(angle=-20))
...
para_limited_tasks <- subset(parasite_over_time, task=="NOT" | task=="NAND" | task=="AND" | task=="ORN" | task=="OR" | task=="ANDN")
