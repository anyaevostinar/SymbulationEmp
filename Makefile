...
OFLAGS_web_all := -s "EXPORTED_RUNTIME_METHODS=['ccall', 'cwrap', 'stringToUTF8', 'UTF8ToString']" -s TOTAL_MEMORY=268435456 --js-library $(EMP_DIR)/emp/web/library_emp.js -s EXPORTED_FUNCTIONS="['_main', '_empCppCallback', '_empDoCppCallback']" -s DISABLE_EXCEPTION_CATCHING=1 -s NO_EXIT_RUNTIME=1 -s ASSERTIONS=1 --embed-file example-settings-cfg/environment.json@/environment.json --embed-file web/sfx/BEEEP.wav@/BEEEP.wav --embed-file example-settings-cfg/spatial-struct.mat@/spatial-struct.mat --embed-file example-settings-cfg/spatial-struct-edges.csv@/spatial-struct-edges.csv
